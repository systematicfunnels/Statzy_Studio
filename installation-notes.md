** Thanks for Son Tran for the fixes on the installation guide. Here are the recommanded steps for installing Statzy Studio:


### You now need Docker to now run Statzy Studio. If you can't run it with docker, please use the Google Colab.
# To run Statzy Studio docker:
...
Export Docker image:
```bash
docker save statzy_studio_docker > statzy_studio_docker.tar
```
...
Statzy Studio will accept this version of FFmpeg:
...
3.2. Download FFmpeg Source Code:

```bash
wget https://ffmpeg.org/releases/ffmpeg-4.2.3.tar.gz
tar -xzf ffmpeg-4.2.3.tar.gz
cd ffmpeg-4.2.3
```
